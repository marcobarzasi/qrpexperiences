...
const getColorRange = () => {
  ...
  if (props.value < levelPerc * 30) {
    return 'orange';
  } else if (props.value > levelPerc * 30 && props.value < levelPerc * 70) {
    return 'dodgerblue';
  } else if (props.value > levelPerc * 70 && props.value < levelPerc * 100) {
    return 'palegreen';
  } else {
    return 'forestgreen';
  }
...
const make = () => {
  window.open('/server/api/Api.php?service=attestati.make&id=QRBVHF.png&level=' + props.maxLevel + '&call=' + globalState.user.call, '_blank');
}
